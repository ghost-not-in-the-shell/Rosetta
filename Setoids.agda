...
module _ {𝑨 𝑩} where
  infix 4 _∼_
  _∼_ : Rel (𝑨 -𝓢𝓮𝓽⟶ 𝑩)
  𝒇 ∼ 𝒈 = ∀ {x y} → 𝑨 ∣ x ∼ y → 𝑩 ∣ 𝒇 ₀ x ∼ 𝒈 ₀ y

  .∼‿refl  : ∀ {𝒇 : 𝑨 -𝓢𝓮𝓽⟶ 𝑩} → 𝒇 ∼ 𝒇
  .∼‿sym   : ∀ {𝒇 𝒈 : 𝑨 -𝓢𝓮𝓽⟶ 𝑩} → 𝒇 ∼ 𝒈 → 𝒈 ∼ 𝒇
  .∼‿trans : ∀ {𝒇 𝒈 𝒉 : 𝑨 -𝓢𝓮𝓽⟶ 𝑩} → 𝒇 ∼ 𝒈 → 𝒈 ∼ 𝒉 → 𝒇 ∼ 𝒉
  ∼‿refl  {𝒇}     = λ x∼y → 𝒇 ₁ x∼y
  ∼‿sym   𝒇∼𝒈     = λ x∼y → sym (𝒇∼𝒈 (sym x∼y))
  ∼‿trans 𝒇∼𝒈 𝒈∼𝒉 = λ x∼y → trans (𝒇∼𝒈 x∼y) (𝒈∼𝒉 refl)

  .∼‿refl₍_₎ : ∀ (𝒇 : 𝑨 -𝓢𝓮𝓽⟶ 𝑩) → 𝒇 ∼ 𝒇
  ∼‿refl₍ 𝒇 ₎ = ∼‿refl {𝒇}
...
{-# DISPLAY _-𝓢𝓮𝓽⟶_ 𝑨 𝑩 = 𝓢𝓮𝓽 ∣ 𝑨 ⟶ 𝑩 #-}
{-# DISPLAY _∼_ 𝒇 𝒈     = 𝓢𝓮𝓽 ∣ 𝒇 ∼ 𝒈 #-}
